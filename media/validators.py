        ...
        try:
            ...
            if mime in self.mime_types:
                if value.size > self.max_size:
                    raise ValidationError(_("Please keep file size under %s. Current file size %s.") % (filesizeformat(self.max_size), filesizeformat(value.size)))
            else:
                raise ValidationError(_("File type %s is not supported.") % mime)
